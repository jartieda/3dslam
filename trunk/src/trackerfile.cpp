...
CTrackerFile::~CTrackerFile()
{
 ...
 void CTrackerFile::Match(IplImage *im){
    ...
    while (fscanf(f,"%d %d %d %d\n", &num, &y, &x,&dumb)!=EOF)
     {
         cout<<"in while"<<endl;
         list<CElempunto*>::iterator It=pEstimator->pMap->bbdd.begin();
         int count=0;
        while  (((*It)->ID!=num)&&( count < pEstimator->pMap->bbdd.size()))
        {
            cout<<"."<<count;
            It++ ;
            count++;
            if (count>= pEstimator->pMap->bbdd.size()) break;
        }
        cout<<"fuera while"<<endl;

        if( count<pEstimator->pMap->bbdd.size()){
		   ...
		   if((*It)->state==st_no_view)
		   {
            ...
            pEstimator->pMap->add_key(cvPoint(x,y),key,getFeatDim());
